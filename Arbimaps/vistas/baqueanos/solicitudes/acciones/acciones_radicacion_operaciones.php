...
$razon  = isset($_POST['sb_razon_pagos']) ? trim($_POST['sb_razon_pagos']) : null;
...
switch ($accion) {
    case 'aprobar':
        $sqlUpdate = "UPDATE solicitud_baqueanos 
                        SET sb_estado_pagos = 'APROBADO',
                            sb_estado_final_pago = 'APROBADO'
                        WHERE id = ?";
        ...
        $mensaje = "La solicitud ARB_{$id} fue APROBADA por gerencia en el proceso de radicación y esta lista para que realices el pago de la cuenta.";

        insertarNotificacionPorRol($mysqli, "pagos", (int)$id, $tipo, $mensaje);

        $_SESSION['alerta'] = 'aprobado';
        header("Location: /arbimaps/Arbimaps/index.php?page=baqueanos/solicitudes/vistas/aprobacion_radicacion_gerencia");
        exit;
    case 'devolver':
        if ($razon === null) {
            $razon = '';
        ...
        $sql = "UPDATE solicitud_baqueanos 
                SET sb_estado_pagos = 'DEVUELTO', 
                    sb_estado_cuenta = 'DEVUELTO', 
                    sb_razon_pagos = ? 
                WHERE id = ?";
        ...
        $stmtUpdate->close();
        $tipo    = "SOLICITUD_BAQUEANOS";
        $mensaje = "La solicitud ARB_{$id} fue DEVUELTA por gerencia en el proceso de radicación de pago."
            . ($razon !== '' ? " Razón: {$razon}" : "");

        insertarNotificacionPorRol($mysqli, "pagos", (int)$id, $tipo, $mensaje);

        $cuenta['sb_estado_pagos'] = 'DEVUELTO';
        $cuenta['sb_estado_cuenta'] = 'DEVUELTO';
        $cuenta['sb_razon_pagos'] = $razon;
        ...
        $mapa = [
            ...
            'sb_razon_pagos'            => 'copia_sb_razon_pagos',

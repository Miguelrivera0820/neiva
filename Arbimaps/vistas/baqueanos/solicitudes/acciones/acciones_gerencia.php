...
$razon  = isset($_POST['sb_razon_gerencia']) ? trim($_POST['sb_razon_gerencia']) : '';
$motivo = $_POST['sb_tipo_motivo'] ?? '';
...
switch ($accion) {
    case 'aprobar':
        $sqlUpdate = "UPDATE solicitud_baqueanos 
                        SET sb_estado_gerencia = 'APROBADO' 
                        WHERE id = ?";
        ...
        $mensaje = "La solicitud ARB_{$id} fue APROBADA por Gerencia, la solicitud esta lista para crear la cuenta de cobro.";

        insertarNotificacionPorRol($mysqli, "social", (int)$id, $tipo, $mensaje);

        $_SESSION['alerta'] = 'aprobado';
        header("Location: /arbimaps/Arbimaps/index.php?page=baqueanos/solicitudes/vistas/detalles_gerencia");
        exit;
    case 'devolver':
        $sql = "UPDATE solicitud_baqueanos 
                SET sb_estado_gerencia = 'DEVUELTO', 
                    sb_razon_gerencia = ?, 
                    sb_tipo_motivo = ? 
                WHERE id = ?";
        ...
        $stmtUpdate->bind_param("ssi", $razon, $motivo, $id);
        ...
        $stmtUpdate->close();
        $tipo    = "SOLICITUD_BAQUEANOS";
        $mensaje = "Su solicitud ARB_{$id} fue DEVUELTA por Gerencia."
    ...
    case 'editar':
        ...
        $sb_dias_calculados     = (int)preg_replace('/[^\d]/', '', $_POST['sb_dias_calculados']);
        $sb_cobro_diario        = (int)preg_replace('/[^\d]/', '', $_POST['sb_cobro_diario']);
        ...
        if ($stmt->execute()) {
            $tipo    = "SOLICITUD_BAQUEANOS";
            $mensaje = "Su solicitud ARB_{$id} fue EDITADA por Gerencia.";
            insertarNotificacionSolicitud($mysqli, (int)$id, $tipo, $mensaje);
            ...
            $stmt->close();
            echo json_encode(['success' => false, 'message' => 'Error al editar la solicitud']);
            exit;
        }
    default:

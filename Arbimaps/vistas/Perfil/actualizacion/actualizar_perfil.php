...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {  
    ...
    if (!empty($password_input)) {
        $password_cons = password_hash($password_input, PASSWORD_DEFAULT);
    } else {
    ...
    if (isset($_FILES['foto_user_file']) && $_FILES['foto_user_file']['error'] !== UPLOAD_ERR_NO_FILE) {
        if ($_FILES['foto_user_file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!array_key_exists($mime, $allowed)) {
                $_SESSION['error_actualizacion'] = 'Tipo de archivo no permitido. Use JPG o PNG.';
                header("Location: " . neiva_app_url('Arbimaps/index.php?page=Perfil/editar_perfil'));
                exit();
            }
    ...
    if ($stmt->execute() && $stmt->affected_rows > 0) {
        ...
        error_log('Perfil actualizar failed: ' . $mysqli->error);
        $_SESSION['error_actualizacion'] = "No se pudo actualizar el perfil. Verifica los datos e intenta nuevamente.";
        header("Location: " . neiva_app_url('Arbimaps/index.php?page=Perfil/editar_perfil'));
        exit();
    }
} else {

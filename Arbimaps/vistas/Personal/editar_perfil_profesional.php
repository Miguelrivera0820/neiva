...
<div class="container-fluid px-3">
    ...
    <div class="card-form p-3">
        ...
        <div class="card-body ">
            <form id="formContratacion" action="<?= neiva_app_url('Arbimaps/vistas/Personal/actualizacion_perfil.php') ?>" method="POST" enctype="multipart/form-data" novalidate>
...
<script>
    (function() {
        ...
        formEditar.addEventListener('submit', function(e) {
            ...
            // ✅ VALIDACIÓN PRIMERO
            const res = validarFormularioCompleto(formEditar);
            if (!res.ok) {
                swalCamposVacios(res.msg || 'No pueden haber campos vacíos o selects sin seleccionar.');
                // Enfocar el campo si existe
                const el = document.getElementById(res.campo) || formEditar.querySelector(`[name="${res.campo}"]`);
                if (el && el.focus) setTimeout(() => el.focus(), 150);
                return;
            }

            // ✅ SI PASA, MUESTRA CONFIRMACIÓN (tu alerta actual)
            Swal.fire({
                customClass: {
                    popup: 'custom-swal-popup',
                    htmlContainer: 'custom-swal-html'
                ...
                didOpen: () => {
                    ...
                    btnYes?.addEventListener('click', () => {
                        Swal.close();
                        formEditar.submit(); // ya pasa validación
                    });
...
<script>
    function getLabelDeCampo(el) {
        ...
        const col = el.closest('.col-md-1, .col-md-2, .col-md-3, .col-md-4, .col-md-5, .col-md-6, .col-md-7, .col-md-8, .col-md-9, .col-md-10, .col-md-11, .col-md-12, .col-12, .col-lg-4, .col-lg-6');
        if (col) {
            const lab2 = col.querySelector('label');
            if (lab2 && lab2.textContent.trim()) return lab2.textContent.trim();
        }
        return el.getAttribute('name') || el.id || 'Este campo';
    }

    function mensajeCampoFaltante(el) {
        const nombre = getLabelDeCampo(el);
        const tag = (el.tagName || '').toLowerCase();
        if (tag === 'select') return `Falta seleccionar: <b>${nombre}</b>.`;
        return `Falta completar: <b>${nombre}</b>.`;
    }
    ...
    function validarFormularioCompleto(form) {
        ...
        for (const el of campos) {
            ...
            if (omitIds.has(el.id) || omitIds.has(el.name)) continue;
        ...
        const selEPS = form.querySelector('#con_eps');
        const otraEPS = form.querySelector('#otra_eps');
        if (selEPS && selEPS.value === 'OTRO') {
            if (!otraEPS || (otraEPS.value ?? '').trim() === '') {
                return {
                    ok: false,
                    campo: 'otra_eps',
                    msg: mensajeCampoFaltante(otraEPS)
                };
            }
        }

        const selIncap = form.querySelector('#incapacitado');
        const inputDis = form.querySelector('#con_discapacidad');
        if (selIncap && selIncap.value === 'si') {
            if (!inputDis || (inputDis.value ?? '').trim() === '') {
                return {
                    ok: false,
                    campo: 'con_discapacidad',
                    msg: mensajeCampoFaltante(inputDis)
                };
            }
        }
        return {
    ...
    function swalCamposVacios(detalle = 'No pueden haber campos vacíos o selects sin seleccionar.') {
        Swal.fire({
            customClass: {
                popup: 'custom-swal-popup',
                htmlContainer: 'custom-swal-html'
            ...
            html: `
                <div class="swal-card">
                    ...
                    <div class="swal-sub-like">${detalle}</div>
                    <div class="swal-actions-like d-flex justify-content-center">

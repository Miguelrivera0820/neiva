...
if (!function_exists('columnaProductoCatastralExiste')) {
    function columnaProductoCatastralExiste($conexion, $tabla, $columna)
    {
...
if (!function_exists('asegurarColumnasProductosCatastrales')) {
    function asegurarColumnasProductosCatastrales($conexion)
    {
        try {
            if (!columnaProductoCatastralExiste($conexion, 'certificado_catastral', 'prod_tipo_producto')) {
                if (!$conexion->query(
                    "ALTER TABLE certificado_catastral
                     ADD COLUMN prod_tipo_producto VARCHAR(100) DEFAULT NULL
                     AFTER cert_area_construccion_predio"
                )) {
                    return false;
                }
            }

            if (!columnaProductoCatastralExiste($conexion, 'certificado_catastral', 'estado')) {
                $estadoInicial = ESTADO_PRODUCTO_SIN_CARGAR;
                if (!$conexion->query(
                    "ALTER TABLE certificado_catastral
                     ADD COLUMN estado VARCHAR(100) NOT NULL DEFAULT '" . $conexion->real_escape_string($estadoInicial) . "'
                     AFTER prod_tipo_producto"
                )) {
                    return false;
                }
            }
...
if (!function_exists('asegurarColumnaProductosCatastrales')) {
    function asegurarColumnaProductosCatastrales($conexion)
    {
        return asegurarColumnasProductosCatastrales($conexion);
    }
}
...
if (!function_exists('asegurarFlujoProductosCatastrales')) {
    function asegurarFlujoProductosCatastrales($conexion)
    {
        ...
        if (!asegurarColumnasProductosCatastrales($conexion)) {
            return false;
        }

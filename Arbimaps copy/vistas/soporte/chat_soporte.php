...
$imgSrc = !empty($solicitud['imagen_ruta'])
    ? '/arbimaps/Arbimaps/vistas/soporte/' . ltrim($solicitud['imagen_ruta'], '/')
    : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'vaciar_chat') {
        ...
        $url_redireccion = '/arbimaps/Arbimaps/index.php?page=soporte/chat_soporte&codigo=' . urlencode($codigo_error);
...
if (!empty($id_solicitante) && !empty($foto_solicitante)) {
    $ruta_foto_fs = $_SERVER['DOCUMENT_ROOT'] . "/arbimaps/Arbimaps/assets/fotos_usuarios/$id_solicitante/$foto_solicitante";
    if (file_exists($ruta_foto_fs)) {
        $foto_solicitante_ruta = "/arbimaps/Arbimaps/assets/fotos_usuarios/$id_solicitante/$foto_solicitante";
...
if (!empty($panel_id) && !empty($panel_foto)) {
    $ruta_panel_fs = $_SERVER['DOCUMENT_ROOT'] . "/arbimaps/Arbimaps/assets/fotos_usuarios/$panel_id/$panel_foto";
    if (file_exists($ruta_panel_fs)) {
        $panel_foto_ruta = "/arbimaps/Arbimaps/assets/fotos_usuarios/$panel_id/$panel_foto";
    ...
    <div class="row chat-header " style="height: 15%; z-index:99">
        <div class="col-12 px-3 d-flex  align-items-center border-bottom">
            ...
            <div class="d-flex align-items-center gap-2 ms-auto">
                ...
                <a href="<?php
                            if ($es_soporte) {
                                echo '/arbimaps/Arbimaps/index.php?page=soporte/base';
                            } else {
                                echo '/arbimaps/Arbimaps/index.php?page=soporte/servicio_cliente';
                            }
                            ?>"
...
<script>
    function cargarMensajes(autoScroll) {
        ...
        xhr.open(
            'GET',
            '/arbimaps/Arbimaps/vistas/soporte/acciones/chat_soporte_mensajes.php?codigo=<?php echo urlencode($codigo_error); ?>',
            true
        );

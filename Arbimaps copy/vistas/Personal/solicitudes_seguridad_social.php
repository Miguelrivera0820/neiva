...
<style>
    ...
    .card-header {
        background: linear-gradient(315deg, #1283B3, #012949ff);
    }
...
<div class="container-fluid">
    <div class="my-4 text-center">
        <h4 class=" mb-0 fw-bold mb-2" style="color: #002F55; font-weight: 700 !important "> MÓDULO DE CARGUE DE OTRO SI</h4>
        <small>A continuación se muestra los registros correspondientes a las solicitudes de otro sí viables financieramente y listas para cargue de otro si.</small>
    </div>
    ...
    <ul class="nav nav-tabs custom-tabs mb-0" id="myTab" role="tablist" style="width: 75%;">
        ...
        <li class="nav-item" role="presentation">
            <button class="nav-link radicadas-tab active d-flex justify-content-center align-items-center"
                ...
                role="tab">
                Revisión
                <span id="contadorRegistros" class="fw-bold"></span>
            </button>
        </li>
    ...
    <div class="tab-content">
                ...
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    ...
                    <div class="d-flex align-items-center ">
                        <div class="d-flex justify-content-center align-items-center me-3 rounded-5  p-2" style="width: 35px; height: 35px; background-color: #ffffffff;">
                            <i class="bi bi-file-earmark-arrow-up" style="color: #002F55;"></i>
                        </div>
                        <div>
                            <div class=" text-start text-white" style="font-size: 1em;  font-weight: 600;">
                                Solicitudes listas para cargar contrato de otro sí
                            </div>
                            <div style="font-size: 70%; color: #f3f8fdff;" class="text-start">
                                Consulta la información las solicitudes de otro sí que ya se encuentran aprobados y listos para enviar al colaborador.
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table table-card " id="dataTable" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                $sql = "SELECT 
                                    ...
                                    so.sol_estado_usuario
                                FROM solicitudes_otrosi AS so
                                JOIN contratacion AS c ON so.con_id = c.con_id
                                WHERE so.sol_estado_gerencia = 'ACEPTADO' 
                                AND (so.sol_estado_cargado = '')
                                ...
                                ?>
                                            ...
                                            <div class="circle-number"><?= $num++ ?></div>
                                                ...
                                                <td class="text-align: center; vertical-align: middle; font-size: 14px;">RAD_<?php echo htmlspecialchars($row['id']); ?></td>
                                                <td class=" text-align: center; vertical-align: middle; font-size: 12px !important;"><?php echo htmlspecialchars($row['sol_fecha_solicitud']); ?></td>
                                                ...
                                                <td class="text-align: center; vertical-align: middle; font-size: 14px;"><?php echo htmlspecialchars($row['sol_duracion']); ?></td>
                                                <td class="text-align: center; vertical-align: middle; font-size: 14px;">$<?php echo number_format((float)($row['sol_nuevo_salario'] ?? 0), 0, ',', '.'); ?></td>
                                                <td class="text-align: center; vertical-align: middle; font-size: 14px;">$<?php echo number_format((float)($row['sol_valor_otrosi'] ?? 0), 0, ',', '.'); ?></td>
                                                <td class="text-align: center; vertical-align: middle; font-size: 14px;"><?php echo htmlspecialchars($row['sol_nuevo_proyecto']); ?></td>
                                                <!-- CONDICONAL PARA EL ESTADO DE LA SOLICITUD POR PARTE DEL COLABORADOR -->
                                                <?php
                                                $estado = $row['sol_estado_usuario'];
                                                ...
                                                <td style="text-align:center; vertical-align:middle; font-size:14px;" class="p-0">
                                                ...
                                                <td class="text-align: center; vertical-align: middle; font-size: 14px;">
                                                    <div class="accion-wrapper">
                                                        <a href="index.php?page=Personal/cargar_talento_humano&id=<?php echo urlencode($row['id']); ?>"
                                                            class="revisar-btn "
                                                            data-bs-toggle='tooltip'
                                                            title="Cargar otro si">
        ...
        <!-- -------------------------tabla de solicitudes de enviadas a colaborador -------------------------  -->
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="me-3">
                            <div class=" text-start text-white" style="font-size: 1em;  font-weight: 700;">
                                Otro si cargados y enviados al colaborador
                            </div>
                            <div style="font-size: 70%; color: #f3f8fdff;" class="text-start">
                                Consulta la información correspondiente a los documentos de otro sí que fueron cargados y envíados al colaborador para su respectiva revisión.
                            </div>
                        </div>
                        ...
                        <table class="table table-card-2" id="dataTable2" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                $sql = "SELECT 
                                    ...
                                    c.con_apellidos,
                                    c.con_num_identidad,  
                                    so.id, so.sol_estado_gerencia, 
                                    so.sol_fecha_solicitud,
                                    so.con_id, so.sol_duracion,
                                    so.sol_nuevo_salario,
                                    so.sol_valor_otrosi,
                                    so.sol_nuevo_proyecto, 
                                    so.sol_estado_usuario
                                ...
                                JOIN contratacion AS c ON so.con_id = c.con_id
                                WHERE so.sol_estado_gerencia = 'ACEPTADO' 
                                AND (so.sol_estado_cargado = 'CARGADO')
                                ORDER BY so.con_id ASC";
                                if ($result = $mysqli->query($sql)) {
                                    $num = 1;
                                    if ($result->num_rows > 0) {
                                ...
                                ?>
                                            ...
                                            <?php
                                            $estado = strtoupper(trim($row['sol_estado_usuario'] ?? ''));

                                            $claseFila = 'estado-desconocido';
                                            if ($estado === 'PENDIENTE') $claseFila = 'estado-pendiente';
                                            elseif ($estado === 'ACEPTADO') $claseFila = 'estado-aprobado';
                                            elseif ($estado === 'RECHAZADO') $claseFila = 'estado-rechazado';
                                            ?>

                                            <tr class="<?php echo $claseFila; ?>" style="text-align:center; vertical-align:middle; font-size:14px;">
                                            ...
                                            <div class="circle-number">
                                                ...
                                                <td class="text-align: center; vertical-align: middle; font-size: 14px;">RAD_<?php echo htmlspecialchars($row['id']); ?></td>
                                                <td class="text-align: center; vertical-align: middle; font-size: 14px;"><?php echo htmlspecialchars($row['sol_fecha_solicitud']); ?></td>
                                                <td class=" nombres text-align: start;  font-size: 14px;"><?= htmlspecialchars($row['con_nombres']) ?> <br>
                                                    <div class="apellidos"> <?= htmlspecialchars($row['con_apellidos']) ?> </div>
                                                </td>
                                                <td class="text-align: center; vertical-align: middle; font-size: 14px;">
                                                    <a href="index.php?page=Personal/informacion_personal&con_num_identidad=<?php echo urlencode($row['con_num_identidad']); ?>"
                                                        class="text-align: center; vertical-align: middle; font-size: 14px;">
                                                        <?php echo htmlspecialchars($row['con_num_identidad']); ?>
                                                    </a>
                                                </td>
                                                ...
                                                <!-- <td><?php echo htmlspecialchars($row['sol_estado_usuario']); ?></td> -->
                                            </tr>
                                        <?php
                                        }
                                    } else {
    ...
    <!-- <div class="card mb-4">
        <div class="card-header">
            <i class="fas fa-table"></i> Solicitudes
        </div>
        <div class="card-body">
            <div class="table-scroll">
                <table class="table table-hover historial-table" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        ?>
                                    <tr>
                                        ...
                                        <td>RAD_<?php echo htmlspecialchars($row['id']); ?></td>
                                        <td><?php echo htmlspecialchars($row['sol_fecha_solicitud']); ?></td>
                                        <td><?php echo htmlspecialchars($row['con_nombres']); ?></td>
                                        <td><?php echo htmlspecialchars($row['con_apellidos']); ?></td>
                                        <td>
                                            <a href="informacion_personal.php?con_num_identidad=<?php echo urlencode($row['con_num_identidad']); ?>"
                                                class="btn btn-link">
                                                <?php echo htmlspecialchars($row['con_num_identidad']); ?>
                                            </a>
                                        </td>
                                        <td><?php echo htmlspecialchars($row['sol_duracion']); ?></td>
                                        ...
                                        <td><?php echo htmlspecialchars($row['sol_nuevo_proyecto']); ?></td>
                                        <td><?php echo htmlspecialchars($row['sol_estado_usuario']); ?></td>
                                        <td>
                                            <a href="index.php?page=Personal/cargar_talento_humano&id=<?php echo urlencode($row['id']); ?>"
                ...
                <div class="container seccion-pendiente">
                    <h2>SOLICITUDES DEL COLABORADOR</h2>
                    <p class="subtitle">
                        A continuación puede consultar todas las solicitudes de Otro Sí por el usuario
                    </p>
                </div>
                <br><br>
                <table class="table table-hover historial-table" id="dataTable" width="100%" cellspacing="0">
...
<script>
    $(document).ready(function() {
        ...
        //Función para leer la cantidad de registros en la tabla y mostrarlo en la pestaña de la card en cada tabla
        function actualizarContador() {
            const total = table.data().count(); // total sin filtro
            const filtrados = table.rows({
                search: 'applied'
            }).count(); // total con filtro/busqueda

            $('#contadorRegistros').text(` (${total})`);
        }

        // Inicial
        actualizarContador();

        // Se actualiza al buscar, paginar, ordenar, etc.

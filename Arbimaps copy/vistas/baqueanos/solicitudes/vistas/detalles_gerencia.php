...
$rolesPermitidos = ["administrador", "director_catastro", "Directivos", "soporte", "gerencia"];
...
/* ================================
   FILTRO UNIFICADO GERENCIA
   ================================ */

$filtroGerencia = "
    sb_estado_gerencia IN ('PENDIENTE', 'APROBADO')
    AND sb_estado_operaciones = 'APROBADO'
    AND (sb_estado_final_pago IS NULL OR sb_estado_final_pago != 'PAGADO')
";

/* ================================
   CONTADOR (MISMO FILTRO)
   ================================ */
...
$sqlCount = "SELECT COUNT(*) AS total
             ...
             WHERE $filtroGerencia";

if ($resCount = $mysqli->query($sqlCount)) {
    $rowCount = $resCount->fetch_assoc();
    $totalAprobados = (int)$rowCount['total'];
}

/* ================================
   RESUMEN APROBADOS / PENDIENTES
   ================================ */
...
$sqlResumen = "SELECT 
                SUM(CASE WHEN sb_estado_gerencia = 'APROBADO' THEN 1 ELSE 0 END) AS total_aprobadas,
                SUM(CASE WHEN sb_estado_gerencia = 'PENDIENTE' THEN 1 ELSE 0 END) AS total_pendientes
              FROM solicitud_baqueanos
              WHERE $filtroGerencia";
...
<div class="container-fluid">
    <div class="row align-items-center mt-2 mb-2">
        <div class="col-12 col-md-6 text-center text-md-center mb-2 mb-md-0">
            <h4 class="mb-0 fw-bold " style="color:#002F55;">GERENCIA</h4>
            <small>Solicitudes que se encuentra en estado de aprobacion</small>
        </div>
        <?php
        ...
        <div class="stepper-wrap w-30">
            <div class="btn-group stepper flex-wrap justify-content-center gap-2 gap-md-0"
                ...
                <button type="button"
                    ...
                    data-step="3"
                    data-href="index.php?page=baqueanos/solicitudes/vistas/aprobacion_radicacion_gerencia">
                    Radicación
                    <span class="badge-custom2"><?php echo $totalAprobados; ?></span>
                </button>
            </div>
        </div>
    </div>
                                ...
                                if ($stmt = $mysqli->prepare($sql2)) {
                                    ...
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                            echo "<tr>";
                                            echo "<td><a href='index.php?page=baqueanos/solicitudes/vistas/informacion_solicitud_gerencia&id=" . urlencode($row['id']) . "' class='rad btn-link'>ARB_" . htmlspecialchars($row['id']) . "</a></td>";
                                            ...
                                            if ($row['sb_estado_dias'] === 'DIAS_APROBADOS') {
                                            ...
                                            if ($row['sb_estado_dias'] === 'DIAS_APROBADOS') {
                                                echo "<i class='fa-solid fa-triangle-exclamation fa-xl' style='color:orange; cursor:pointer;' 
                                                        ...
                                                        data-id='" . $row['id'] . "' 
                                                        title='Se han aprobado días adicionales en esta solicitud'></i>";
                                            } else {
                                                echo ($row['sb_estado_gerencia'] === 'APROBADO'
                                    ...
                                    echo "<tr><td colspan='10' class='text-center'>Error al preparar la consulta: " . htmlspecialchars($mysqli->error) . "</td></tr>";
                                }
                                ?>
                            </tbody>
                        </table>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        fetch("cargar_notificaciones.php?tipo=gerencia")
        ...
        <div class="swal-saved-card">
            ...
            <div class="swal-saved-body">
                <i class="bi bi-exclamation-circle-fill text-danger"
                    style="font-size:75px;opacity:.15;"></i>
                <div class="swal-saved-text">
                    La solicitud ha sido devuelta correctamente.
                </div>
                <div class="mt-4">
                    <button onclick="Swal.close()" 
                        class="btn-swal-primary">

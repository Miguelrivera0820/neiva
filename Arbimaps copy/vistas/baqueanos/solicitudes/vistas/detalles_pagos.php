...
//Consulta para contar las cuentas devueltas
$sqlCount = "SELECT COUNT(*) AS total FROM solicitud_baqueanos WHERE sb_estado_cuenta = 'DEVUELTO'";
$result = $mysqli->query($sqlCount);
$row = $result->fetch_assoc();
$totalRadicadas = $row['total'];

//Consulta para contar las cuentas aprobadas por jefe de operaciones
$sqlCount = "SELECT COUNT(*) AS total FROM solicitud_baqueanos WHERE sb_estado_pagos = 'APROBADO'";
$result = $mysqli->query($sqlCount);
$row = $result->fetch_assoc();
$totalCargado = $row['total'];
...
<div class="container-fluid">
    <div class="my-4 text-center">
        <h4 class=" mb-0 fw-bold mb-2" style="color: #002F55; font-weight: 700 !important ">SEGUIMIENTO DE RADICACIONES DE SOLICITUDES</h4>
        <small>Solicitudes que cuentan con una cuenta de cobro ya radicada</small>
    </div>
        ...
        <div class="stepper-wrap w-30">
            <div class="btn-group stepper flex-wrap justify-content-center gap-2 gap-md-0"
                ...
                <button type="button"
                    ...
                    data-step="2"
                    data-href="index.php?page=baqueanos/solicitudes/vistas/detalles_cargar_soporte">
                    Soporte pago
                    <span class="badge-custom"><?php echo $totalCargado; ?></span>
                </button>
            </div>
        </div>
    </div>
                    ...
                    <div class="d-flex align-items-center ">
                        ...
                        <div>
                            ...
                            <div style="font-size: 70%; color: #f3f8fdff;" class="text-start">
                                Estado de las radicaciones de las solicitudes
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table  table-card text-center align-middle" id="dataTable" width="100%" cellspacing="0">
                            <thead>
                                <tr class="text-center">
                                    ...
                                    <th style="text-align: center; vertical-align: middle; font-size: 12px">Jefe operaciones</th>
                                    <th style="text-align: center; vertical-align: middle; font-size: 12px">Gerencia</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                $sql2 = "SELECT 
                                        ...
                                        WHERE sb_estado_cuenta IN ('RADICADO', 'APROBADO', 'DEVUELTO') OR
                                                sb_estado_pagos IN ('RADICADO', 'APROBADO', 'DEVUELTO') 
                                        ORDER BY id ASC
                                    ";
                                if ($stmt = $mysqli->prepare($sql2)) {
                                    ...
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            $sqlExt = "SELECT SUM(dias_agregados) AS suma_dias, SUM(valor_adicional) AS suma_valor FROM solicitud_baqueanos_extensiones WHERE solicitud_id = ? AND estado = 'REGISTRADO'";
                                            $stmtExt = $mysqli->prepare($sqlExt);
                                            $stmtExt->bind_param("i", $row['id']);
                                            $stmtExt->execute();
                                            $ext = $stmtExt->get_result()->fetch_assoc();
                                            $stmtExt->close();

                                            $diasTotal = $row['sb_dias_calculados'] + ($ext['suma_dias'] ?? 0);
                                            $valorTotal = $row['sb_valor_cobrar'] + ($ext['suma_valor'] ?? 0);

                                            echo "<tr>";
                                            echo "<td><a href='index.php?page=baqueanos/cuentas/vistas/informacion_solicitud_radicacion&id=" . urlencode($row['id']) . "' class='rad btn-link'>ARB_" . htmlspecialchars($row['id']) . "</a></td>";
                                            ...
                                            echo "<td>" . htmlspecialchars($row['sb_numero_identidad']) . "</td>";
                                            echo "<td>" . htmlspecialchars($diasTotal) . "</td>";
                                            echo "<td>$" . number_format($row['sb_cobro_diario'], 0, ',', '.') . "</td>";
                                            echo "<td>$" . number_format($valorTotal, 0, ',', '.') . "</td>";
                                            ...
                                            echo "<td class='text-center'>" .
                                                ($row['sb_estado_pagos'] === 'APROBADO'
                                                    ? '<i class="fa-solid fa-circle-check fa-xl" style="color: rgb(24, 167, 22);"></i>'
                                                    : ($row['sb_estado_pagos'] === 'PENDIENTE'
                                                        ? '<i class="fa-solid fa-spinner fa-xl fa-spin" style="color:rgb(56, 133, 195);"></i>'
                                                        : '<i class="fa-solid fa-circle-xmark fa-xl" style="color: rgb(200, 30, 30);"></i>')
                                                ) . "</td>";
                                            echo "</tr>";

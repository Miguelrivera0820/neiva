...
if (!$numero_identidad && !$id_get) {
    echo json_encode(["success" => false, "message" => "Número de identidad o id no proporcionado"]);
    exit();
}
...
$select_fields = "
                ...
                razon";
if ($id_get) {
    $query = "SELECT id, $select_fields FROM cuentas_rechazadas WHERE id = ? LIMIT 1";
    ...
    $query = "SELECT $select_fields FROM cuentas_rechazadas WHERE numero_identidad = ? ORDER BY fecha_rechazo DESC LIMIT 1";
    $stmt = $mysqli->prepare($query);
    if (!$stmt) {
        echo json_encode(["success" => false, "message" => "Error en la consulta SQL: " . $mysqli->error]);
        exit();
...
if ($result && $result->num_rows > 0) {
    ...
    $numero_identidad       = $row['numero_identidad']      ?? $numero_identidad;
        ...
        <div class="col-12">
            ...
            <div class="card shadow col-12 mt-5 px-2 mb-4 border rounded-4" style=" position:relative;  overflow:visible; border: 1px solid #05518F !important">
                ...
                <div class="row p-4 pt-4">
                    ...
                    <div class="col-md-6 col-lg-4 p-1 px-2 my-3">
                        <label for="rec_anio_cuenta" class="form-label" style="font-size:0.9em;"><b>Periodo Seleccionado</b></label>
                        <div class="input-group shadow-sm rounded-2 <?php echo (!empty($rec_anio_cuenta)) ? 'precargado' : ''; ?>">
                            <span class="input-group-text" id="basic-addon1"><i class="bi bi-calendar-week"></i></span>
                            <input type="text" style="font-size:0.9em;" class="form-control" id="rec_anio_cuenta" placeholder="Ingresa valor ..."
...
<script>
    (function() {
        ...
        const dias = document.getElementById('cant_dias');
        ...
        function calc() {
            ...
            const ms = d2 - d1;
            dias.value = Math.floor(ms / 86400000) + 1;
        }
